...
export default function ProductCart() {
    ...
    const removeFromCart = (product) => {
        ...
        if (checkoutItems.some(item => item.productId === product.productId
            && item.color === product.color
            && item.size === product.size)) {
    ...
    const getCart = () => {
        ...
        for (let item of cartItems) {
            const product = productItem.find(product =>
        ...
        setCart(prevCart => {
            const updatedCart = prevCart.map(item => {
                if (item.productId === product.productId &&
                    ...
                    axiosClient.put('/cart/updateQuantity', {
                        productId: item.productId,
                        color: item.color,
    ...
    const handleSelectCartItem = (item) => {
        if (checkoutItems.some(it => it.productId === item.productId
            && it.color === item.color
            && it.size === item.size)) {
    ...
    const removeCheckoutItem = (item) => {
        ...
        const newCheckout = tmp.filter(it => !(it.productId === item.productId
            && it.color === item.color
            && it.size === item.size))
                                    ...
                                    <div className={cx("cart-header-info")}>
                                        <div className={cx("cart-check-box-header")}>
                                            <input
                                                ...
                                                checked={checkoutItems.length === cart.length ? true : false}
                                            />
                                        </div>
                                        ...
                                        <div className={cx("cart-row")}>
                                            {
                                                cart.map((item, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className={cx("cart-product-image")} title="">
